 ...
 * request headers, request authorization, request signatures, response
 ...
 * with is closed and discarded, [close] must be called on the backend to ensure
 * any resources it is using are released.
 */
interface Backend {
    /**
     * Gets the backend service's base URL (service endpoint) identifying the
     * service that will be accessed by this backend.
     */
    fun baseUrl(): String

    /**
     * Prepares the request for use by the backend service. Changes may be made
     * to the request to adapt it for backends other than the default Anthropic
     * backend.
     *
     ...
     * from the default Anthropic backend. All changes that may affect the
     * resolution of the full URL for the request _must_ be made by this method.
     *
     ...
     * This is the behavior of the default implementation.
     */
    ...
    /**
     * Authorizes the given request and returns the new authorized request. The
     * URL of the request will be resolved fully _before_ this method is called.
     *
     * Changes will typically include the addition of authorization headers or
     * signature headers. Other changes not directly related to authorization
     * should be performed by [prepareRequest].
     *
    ...
    /**
     * Closes any streams and releases any resources used by this backend. For
     * example, an implementation might use this to shut down a thread pool that
